...
CDIPCVnetVOSPortLocal::CDIPCVnetVOSPortLocal(CDIPCVnetVOSManager* switch_manager)
		:m_tap_init(false),m_switch_manager(switch_manager)
{
}

CDIPCVnetVOSPortLocal::~CDIPCVnetVOSPortLocal()
{	
}

//CAWTap OnOutput input to switch_manager
void CDIPCVnetVOSPortLocal::OnOutput(const char* pkt, size_t pktsize)
{
    CAW_INFO_TRACE("CDIPCVnetVOSPortLocal::OnOutput Tap Input IDIPCVSwitch datasize="<<pktsize);
	m_switch_manager->GetVOSPort()->Input(pkt,pktsize);
}


CAWResult CDIPCVnetVOSPortLocal::Tap_init()
{
	if(m_tap_init==true)
	{	
		return CAW_OK;
	}
	CAW_INFO_TRACE("CDIPCVnetVOSPortLocal::Tap_init");

    CAWString strdevname=CDipcVnetConfig::Instance()->GetOsTapName();

    CAWResult rv;
    DWORD extos_ip = 0;
    CAWString strip = CDipcVnetConfig::Instance()->GetOsTapIp();
    if(FALSE == CAWInetAddr::IpAddrStringTo4Bytes(strip.c_str(), extos_ip))
    {
        CAW_ERROR_TRACE("CDIPCVnetVOSPortLocal::Tap_inits IP address translation failed");
    ...
    if (CAW_FAILED(rv))
    {
        CAW_ERROR_TRACE("CDIPCVnetVOSPortLocal::Tap_inits Init failed");
        return CAW_ERROR_FAILURE;
    }

    DWORD extos_netmask = 0;
    CAWString strnetmask = CDipcVnetConfig::Instance()->GetOsTapMask();
    CAWInetAddr::IpAddrStringTo4Bytes(strnetmask.c_str(), extos_netmask);
    rv = SetVPortIPNetmask((uint32_t)ntohl(extos_netmask));
    if (CAW_FAILED(rv))
    {
        CAW_ERROR_TRACE("CDIPCVnetVOSPortLocal::Tap_inits SetVPortIPNetmask failed "<<extos_netmask);
    }

    DWORD extos_gateway = 0;
    CAWString strgateway = CDipcVnetConfig::Instance()->GetOsTapGateway();
    CAWInetAddr::IpAddrStringTo4Bytes(strgateway.c_str(), extos_gateway);
    rv = SetVPortIPGateway((uint32_t)ntohl(extos_gateway));
    if (CAW_FAILED(rv))
    {
        CAW_ERROR_TRACE("CDIPCVnetVOSPortLocal::Tap_init SetVPortIPGateway failed "<<extos_gateway);
    }

    rv = Up();
    if (CAW_FAILED(rv))
    {
        CAW_ERROR_TRACE("CDIPCVnetVOSPortLocal::Tap_init Up() failed");
    }
